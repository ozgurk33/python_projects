...
def generate_cv_pdf():
    ...
    if not name or not email or not phone or not address or not website or not skills or not work_experience or not education or not about_me:
...
label_education = Label(window, text = 'Education(Enter one education per line in format "Degree": "University")')
label_education.pack()
entry_education = Text(window, height = 5)
entry_education.pack()

label_experience = Label(window, text = 'Experience(Enter one experience per line in format "Job Title": "Description")' )
